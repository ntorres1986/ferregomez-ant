<?php
    ...
    class PDF extends FPDF
    {
        ...
        { 
           ...
           $this->Cell($this->widths[2],10,"RECIBO",1,0,'L',true);
           $this->Cell($this->widths[3],10,"TRABAJADOR",1,0,'L',true);
           $this->Cell($this->widths[4],10,"CLIENTE",1,0,'L',true);
    ...
    $widths = array(30,30,30,80,80,30);
    ...
    if( empty($fin) )
      $title = "REPORTE DE CUENTAS POR COBRAR ($inicio)";
    else
      $title = "REPORTE DE CUENTAS POR COBRAR DE ($inicio A $fin)";
    ...
    if( count($results) > 0 )
    {
         ...
         foreach ($results as $row ) 
         {
             ...
             $pdf->Cell($pdf->widths[2],8,$row['recibo'],1,0,'L',true);
             $pdf->Cell($pdf->widths[3],8,$row['trabajador'],1,0,'L',true);
             $pdf->Cell($pdf->widths[4],8,$row['cliente'],1,0,'L',true);
   ...
   if(file_exists("ReporteCuentasPorCobrar.pdf") )
     unlink("ReporteCuentasPorCobrar.pdf");
      

   $pdf->Output("ReporteCuentasPorCobrar.pdf","F");
   echo "reports/ReporteCuentasPorCobrar.pdf";
