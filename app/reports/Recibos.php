<?php
        ...
        $sql = "SELECT current_timestamp fecha";
        ...
        $pdf->AddPage(); 
        $pdf->Image('ferregomez_encabezado.png' , 1.5 ,0.4, 2 ,.6,'PNG');
        $pdf->SetY(0);
        ...
        $pdf->Cell(.5,.1,"Fecha :",1,0,'L',true);  
        $pdf->SetXY(1.9 ,1.2);
        $pdf->SetFillColor(255,255,255);
        $pdf->SetDrawColor(223,223,223);

        $pdf->Cell(1.2,.1,"$day de $month de $year ",1,1,'L',true); 
        
        $pdf->SetFillColor(223,223,223);        
        $pdf->SetDrawColor(223,223,223);
        $pdf->SetXY(1.4 , 1.32);
        $pdf->Cell(.5,.1,utf8_decode("Señor(es) "),1,0,'L',true); 

        $pdf->SetFillColor(255,255,255);
        

        $pdf->SetXY(1.9 ,1.32);
        $pdf->Cell(1.2,.1,utf8_decode($cliente),1,0,'L',false); 


        $pdf->SetFillColor(223,223,223);        
        $pdf->SetDrawColor(223,223,223);
        $pdf->SetXY(3.2 ,1.32);
        $pdf->Cell(.5,.1,utf8_decode("Teléfono"),1,0,'L',true); 
        $pdf->SetFillColor(255,255,255);
        $pdf->SetXY(3.7 ,1.32);
        $pdf->Cell(.8,.1,utf8_decode($telefono),1,1,'L',true); 

        $pdf->SetXY(1.4 ,1.44);

        $pdf->SetFillColor(223,223,223);        
        $pdf->SetDrawColor(223,223,223);
        $pdf->Cell(.5,.1,utf8_decode("Dirección"),0,0,'L',true); 
        $pdf->SetXY(1.9 ,1.44); 
        $pdf->SetFillColor(255,255,255);
        $pdf->Cell(1.2,.1,"Direadsad " .utf8_decode($direccion),1,0,'L',true); 

 
        $pdf->SetFillColor(223,223,223);        
        $pdf->SetDrawColor(223,223,223);
        $pdf->SetXY(3.2 ,1.44); 
        $pdf->Cell(.5,.1,utf8_decode("Pago"),0,0,'L',true); 
        $pdf->SetFillColor(255,255,255);

        $pdf->SetXY(3.7 ,1.44); 
        $pdf->Cell(.8,.1,$tipopago,1,0,'L',true);  
        ...
        echo json_encode( array( "pdf_uri" => "Recibos$prefijo.pdf"  ) );   
